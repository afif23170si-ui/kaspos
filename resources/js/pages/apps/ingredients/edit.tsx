...
export default function Create() {
  ...
  const { material, units } = usePage<EditProps>().props;

  const { data, setData, errors, processing, post, reset } = useForm({
    name: material.name,
    unit_id: material.unit_id.toString(),
    expired_at: material.initial_stock.expired_at,
    initial_qty: material.initial_stock.quantity,
  ...
  const storeData = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    post(route('apps.materials.update', material.id), {
      onSuccess: () => {
      ...
      <div className='p-6'>
        <Card>
          <CardHeader>
            ...
            <CardDescription>This form for edit ingredient</CardDescription>
          </CardHeader>
          <CardContent>
