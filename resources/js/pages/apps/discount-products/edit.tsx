...
export default function Edit() {
    const { items, customers, discountProduct } = usePage<EditProps>().props;

    const {data, setData, post, reset, processing, errors} = useForm({
        item_id: '',
        customer_id: '',
        is_active: discountProduct.is_active ? '1' : '0',
        discount_type: discountProduct.discount_type == 'nominal' ? 'nominal' : 'precent',
        discount_value: discountProduct.discount_value ?? '',
        discount_quantity: discountProduct.discount_quantity ?? '',
        discount_name: discountProduct.discount_name ?? '',
        is_all_products: discountProduct.all_customers ? 'semua' : 'tertentu',
        is_all_customers: discountProduct.all_products ? 'semua' : 'tertentu',
        selectedCustomers: discountProduct.discount_product_customers.map((customer) => {
            return {
        ...
        selectedItems: discountProduct.discount_product_items.map((item) => {
            return {
                id: String(item.id),
    ...
    const addItem = (itemParams: string, type: string) => {
        ...
        if(selectedProduct){
            const newItem = {
                ...
                type: String(selectedProduct.type)
            }
        ...
        if(selectedCustomer){
            ...
            const isDuplicate = Array.isArray(data.selectedCustomers) && data.selectedCustomers.some(item => item.name === newItem.name);
    ...
    const storeData = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        post(route('apps.discount-products.update', discountProduct.id), {
            onSuccess: () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title='Ubah Diskon Produk'/>
            <div className='p-6'>
                <form onSubmit={storeData}>
                    <Card>
                        <CardHeader>
                            <CardTitle>Ubah Diskon Produk</CardTitle>
                            <CardDescription>Form ini digunakan untuk mengubah data diskon produk</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <Input value={data.discount_quantity} onChange={(e) => setData('discount_quantity', String(e.target.value))} placeholder="Contoh: 3" />
                            ...
                            {data.selectedItems.length > 0 &&
                                <div className="space-y-2 pt-4">
                                    ...
                                    <div className="overflow-auto border rounded-md border-b-0">
